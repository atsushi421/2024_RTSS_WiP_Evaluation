...
def visualize_graph_top_down_more_spacing(yaml_path):
    ...
    # Add nodes and store execution_time as attribute
    for node in perception_data['nodes']:
        G.add_node(node['id'], name=node['name'], execution_time=node['execution_time'])
    ...
    labels = {
        node: f"{G.nodes[node]['name']}\n(execution_time: {G.nodes[node]['execution_time']}ms)"
        for node in G.nodes()}
    nx.draw(G, pos, labels=labels, node_size=1500, node_color='skyblue',
            font_size=6, font_weight='bold', width=1.5, edge_color='gray')
    plt.title(f'Top-Down Graph Structure with execution_time of {os.path.basename(yaml_path)}')
